...
export async function editAnnotationsWithChatGPT(
  ...
  if (!context.layout?.points?.length) {
    return context.existingAnnotations ?? [];
  }
  ...
  const editPrompt =
    BASE_SYSTEM_PROMPT +
    ' You are now editing existing annotations. Apply ONLY the requested changes from editTranscript. ' +
    'You may move, delete, or add annotations when explicitly asked. Return the FULL updated annotations array.';
  ...
  const parsed = await runPlannerCompletion(
    editPrompt,
    'Apply editTranscript to the existing annotations. Only change placements explicitly mentioned. ' +
      'Return the full annotations array (objects[]) after edits.',
    instructions,
  );

  return (parsed.objects ?? []).map((obj, index) => ({
    id: obj?.label ? normalizeId(obj.label) : `ai_edit_${index}_${randomId()}`,
    type: obj.type?.trim() || 'unknown',
    x: obj.x,
    y: obj.y,
    label: obj.label?.trim(),
